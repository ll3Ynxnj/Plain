...
protected:
  ...
  std::vector<PLAActor *> _actors = std::vector<PLAActor *>(0);

  virtual void SetUp()
  {
    _rootActor = new PLARectActor(kGRARectNorm, kGRAColorWhite);
...
{
  EXPECT_EQ(0, this->_rootActor->GetSizeOfActors());
  this->_rootActor->AddActor(_actors[0]);
  EXPECT_EQ(1, this->_rootActor->GetSizeOfActors());
  this->_rootActor->AddActor(_actors[1]);
  EXPECT_EQ(2, this->_rootActor->GetSizeOfActors());
}
